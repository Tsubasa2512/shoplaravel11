<tr>
    ...
    <td class="border  border-gray-300 p-1">
        ...
        <a href ="{{ route('admin.product-menu.edit', ['id' => $item->id]) }}" title="Edit Product Menu"
            class="inline-block ml-4 hover:opacity-70 hover:scale-125 ">
            <x-icons.edit class="size-7 fill-sky-600 " />
        </a>
        <form action="{{ route('admin.product-menu.delete', $item->id) }}" method="POST" class=" inline-block">
            @csrf
            @method('DELETE')
