...
@section('content')
    ...
    <div class="container-add-admin p-4">
        ...
        <form action="{{ route('admin.article-menu.update', $articleMenu->id) }}" method="POST" enctype="multipart/form-data"
            class="form flex flex-col">
            {{ csrf_field() }}
            <div class="box--data-admin flex mb-4 text-sm  items-center">
                <label class="w-40 font-semibold">Name </label>
                <input type="text" name="name" required maxlength="255" value="{{ $articleMenu->name }}"
            ...
            <div class="box--data-admin flex mb-4 text-sm  items-center">
                ...
                <select name="category_id" required
                    ...
                    @foreach ($category as $item)
                        <option value="{{ $item->id }}" {{ $item->id == $articleMenu->category_id ? 'selected' : '' }}>
                            {{ $item->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="box--data-admin flex mb-4 text-sm  items-center">
                ...
                <select name="parent_id" required
                    ...
                    @foreach ($parents as $item)
                        <option value="{{ $item->id }}" {{ $item->id == $articleMenu->parent_id ? 'selected' : '' }}>
                            {{ $item->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="box--data-admin flex mb-4 text-sm  items-center">
                ...
                <input type="text" name="index_menu" maxlength="5" value="{{ $articleMenu->index_menu }}"
                    class="p-1 px-4 flex-1 shadow-lg rounded focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 ">
            </div>
            ...
            @if ($articleMenu->image)
                <div class="box--data-admin flex mt-2 mb-4 text-sm  items-center">
                    ...
                    <div class="show--image">
                        <input type="hidden" name="image_old" value="{{ $articleMenu->image }}">
                        <p class="text-sm text-gray-500 mb-4">Current File: {{ $articleMenu->image }}</p>
                        <div class="p-3 shadow-lg w-fit rounded border border-sky-500">
                            <img src="{{ asset('storage/' . $articleMenu->image) }}" alt="Category Image"
            ...
            <div class="box--data-admin ">
                <div class="flex mt-2 mb-4 text-sm  items-center">
                    <label class="inline-flex items-center mb-5 cursor-pointer">
                        ...
                        <input name="show" type="checkbox"class="sr-only peer " value="{{ $articleMenu->show }}"
                            {{ $articleMenu->show ? 'checked' : '' }}>
                        <div
                ...
                <div class="flex mt-2 mb-4 text-sm  items-center">
                    <label class="inline-flex items-center mb-5 cursor-pointer">
                        ...
                        <input name="featured" type="checkbox" class="sr-only peer " value="{{ $articleMenu->featured }}"
                            {{ $articleMenu->featured ? 'checked' : '' }}>
                        <div
                            class="outline-none ring-4 ring-blue-300 relative w-16 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700
                                peer-checked:after:translate-x-10 rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white
                                after:content-[''] after:absolute after:top-0 after:start-0after:bg-white after:border-gray-300 after:border after:rounded-full after:h-6 after:w-6 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600">
            ...
            <div class="box--data-admin flex mb-4 text-sm items-center">
                ...
                <textarea name="description"
                    class="p-1 px-4 h-36 flex-1 shadow-lg rounded focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500">{{ $articleMenu->description }}</textarea>
            </div>
            <div class="box-btn--save flex mt-3">
                ...
                <a href="{{ route('admin.article-menu') }}" title="Article Menu"
